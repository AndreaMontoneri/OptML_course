...
def generate_dataset(name, n_samples=200):
    ...
    if name == "blobs":
        coordinates, labels = sklearn.datasets.make_blobs(
            n_samples=n_samples, centers=2, random_state=SEED
        )
    elif name == "moons":
        coordinates, labels = sklearn.datasets.make_moons(
            n_samples=n_samples, random_state=SEED
        )
        coordinates[labels == 1] += 0.1
        coordinates[labels == 0] -= 0.1
    elif name == "circles":
        coordinates, labels = sklearn.datasets.make_circles(
            n_samples=n_samples, random_state=SEED
        )
        ...
        coordinates = np.stack(
            [x_coordinate.reshape(-1), y_coordinate.reshape(-1)], axis=-1
        )
    ...
    train, test = torch.utils.data.random_split(
        dataset, [n_samples // 2, n_samples // 2]
    )
    return train, test
...
def predict_grid(
    model, ax, xmin=-0.1, xmax=1.1, ymin=-0.1, ymax=1.1, num_grid_points=40
):
    x_coordinate, y_coordinate = np.meshgrid(
    ...
    cmap = LinearSegmentedColormap.from_list(
        "bada55_dark", ["#4d5b23", "#234d5b"], N=100
    )
